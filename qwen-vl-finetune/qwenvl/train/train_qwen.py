...
def set_model(model_args, model):
    ...
    if model_args.tune_moe:
        for n, p in model.model.named_parameters():
            if 'experts_scaler' in n or 'mlp_gate' in n or 'moe_block' in n:
                p.requires_grad = True
    else:
        for n, p in model.model.named_parameters():
            if 'experts_scaler' in n or 'mlp_gate' in n or 'moe_block' in n:
...
def train(attn_implementation="flash_attention_2"):
    ...
    if "qwen" in model_args.model_name_or_path.lower():
        model_name = json.load(open(os.path.join(model_args.model_name_or_path, 'config.json')))['architectures'][0]
    ...
    source_path = os.path.join(
        model_args.model_name_or_path, "chat_template.json"
    )
    template_path = os.path.join(
        training_args.output_dir, "chat_template.json"
    )
