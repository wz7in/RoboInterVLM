...
from .image_reader import get_reader
...
class LazySupervisedDataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            img_tokens = 128 if "images" in sample else 0 # FIX BUG
            length_list.append(
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = (
                cur_len if ("images" in sample) or ("video" in sample) else -cur_len # FIX BUG
            )
    ...
    def process_image_unified(self, image_file, is_local, resize):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        video = None
        if "images" in sources[0] and len(sources[0]["images"]): #@jinhui here is a bug
            image_folder = self.list_data_dict[i]["data_path"]
            image_file = self.list_data_dict[i]["images"]
            resize = self.list_data_dict[i].get("resize", None)
            is_local = self.list_data_dict[i].get("is_local", False)
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    image_file = [
                        os.path.join(image_folder, file) for file in image_file
                    ]
                    results = [self.process_image_unified(file, is_local, resize) for file in image_file]
                    image, grid_thw = zip(*results)
                else:
                    image_file = image_file[0]
                    image_file = os.path.join(image_folder, image_file)
                    image, grid_thw = self.process_image_unified(image_file, is_local, resize)
                    image = [image]
            else:
                image_file = os.path.join(image_folder, image_file)
                image, grid_thw = self.process_image_unified(image_file, is_local, resize)
        ...
        if "images" in self.list_data_dict[i]: # FIX BUG
